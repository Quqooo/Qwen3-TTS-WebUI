        ...
        <section class="flex-1 min-w-0 min-h-0 border rounded-xl bg-card overflow-hidden relative flex flex-col">
          ...
          <div class="flex-1 min-h-0 overflow-y-auto px-5 py-5">
            ...
            <div v-else-if="activeSection === 'batch'" class="max-w-4xl space-y-4">
              ...
              ]" :key="field[0]" class="space-y-1.5" :class="field[0] === 'max_segments' && 'md:col-span-2'"><label :for="`settings-batch-${field[0]}`" class="text-xs text-muted-foreground">{{ $t(`views.settings.${field[1]}`) }}</label><input :id="`settings-batch-${field[0]}`" :name="`batch_${field[0]}`" v-model.number="batchComposer[field[0] as keyof BatchComposerSettings]" type="number" :min="field[2]" :max="field[3]" :step="field[4]" class="w-full px-3 py-2 text-sm" /></div></div>
            </div>
                ...
                <div class="border rounded-xl bg-card p-4 space-y-4">
                  ...
                  <section class="space-y-3">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                      <div class="space-y-1.5"><div class="text-[10px] text-muted-foreground">{{ $t('components.generationParams.doSample') }}</div><AppSelect compact :model-value="samplingValue(defaultParams[selectedKind].do_sample)" :options="samplingOptions" @update:model-value="setSampling('do_sample', $event)" /></div>
                      <div class="space-y-1.5"><label for="settings-default-repetition-penalty" class="text-[10px] text-muted-foreground">{{ $t('components.generationParams.repPenalty') }}</label><input id="settings-default-repetition-penalty" name="default_repetition_penalty" type="number" min="0.01" max="10" step="0.01" :value="paramValue('repetition_penalty')" placeholder="1.05" class="param-input" @change="setNumber('repetition_penalty', ($event.target as HTMLInputElement).value, 0.01, 10)" /></div>
                    </div>
                    <div class="grid grid-cols-3 gap-3">
                      <div class="space-y-1.5"><label for="settings-default-top-k" class="text-[10px] text-muted-foreground">{{ $t('components.generationParams.topK') }}</label><input id="settings-default-top-k" name="default_top_k" type="number" min="0" max="32767" step="1" :value="paramValue('top_k')" placeholder="50" :disabled="defaultParams[selectedKind].do_sample === false" class="param-input" @change="setNumber('top_k', ($event.target as HTMLInputElement).value, 0, 32767, true)" /></div>
                      <div class="space-y-1.5"><label for="settings-default-top-p" class="text-[10px] text-muted-foreground">{{ $t('components.generationParams.topP') }}</label><input id="settings-default-top-p" name="default_top_p" type="number" min="0.01" max="1" step="0.01" :value="paramValue('top_p')" :disabled="defaultParams[selectedKind].do_sample === false" class="param-input" @change="setNumber('top_p', ($event.target as HTMLInputElement).value, 0.01, 1)" /></div>
                      <div class="space-y-1.5"><label for="settings-default-temperature" class="text-[10px] text-muted-foreground">{{ $t('components.generationParams.temperature') }}</label><input id="settings-default-temperature" name="default_temperature" type="number" min="0.1" max="10" step="0.1" :value="paramValue('temperature')" :disabled="defaultParams[selectedKind].do_sample === false" class="param-input" @change="setNumber('temperature', ($event.target as HTMLInputElement).value, 0.1, 10)" /></div>
                    </div>
                  </section>
                  <section class="border-t pt-3 space-y-3">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                      <div class="space-y-1.5"><div class="text-[10px] text-muted-foreground">{{ $t('components.generationParams.doSample') }}</div><AppSelect compact :model-value="samplingValue(defaultParams[selectedKind].subtalker_dosample)" :options="samplingOptions" @update:model-value="setSampling('subtalker_dosample', $event)" /></div>
                      <div class="space-y-1.5"><label for="settings-default-subtalker-top-k" class="text-[10px] text-muted-foreground">{{ $t('components.generationParams.topK') }}</label><input id="settings-default-subtalker-top-k" name="default_subtalker_top_k" type="number" min="0" max="32767" step="1" :value="paramValue('subtalker_top_k')" placeholder="50" :disabled="defaultParams[selectedKind].subtalker_dosample === false" class="param-input" @change="setNumber('subtalker_top_k', ($event.target as HTMLInputElement).value, 0, 32767, true)" /></div>
                    </div>
                    <div class="grid grid-cols-2 gap-3">
                      <div class="space-y-1.5"><label for="settings-default-subtalker-top-p" class="text-[10px] text-muted-foreground">{{ $t('components.generationParams.topP') }}</label><input id="settings-default-subtalker-top-p" name="default_subtalker_top_p" type="number" min="0.01" max="1" step="0.01" :value="paramValue('subtalker_top_p')" :disabled="defaultParams[selectedKind].subtalker_dosample === false" class="param-input" @change="setNumber('subtalker_top_p', ($event.target as HTMLInputElement).value, 0.01, 1)" /></div>
                      <div class="space-y-1.5"><label for="settings-default-subtalker-temperature" class="text-[10px] text-muted-foreground">{{ $t('components.generationParams.temperature') }}</label><input id="settings-default-subtalker-temperature" name="default_subtalker_temperature" type="number" min="0.1" max="10" step="0.1" :value="paramValue('subtalker_temperature')" placeholder="0.9" :disabled="defaultParams[selectedKind].subtalker_dosample === false" class="param-input" @change="setNumber('subtalker_temperature', ($event.target as HTMLInputElement).value, 0.1, 10)" /></div>
                    </div>
                  </section>
                  <section class="border-t pt-3 space-y-3">
                    <div class="text-xs font-medium text-muted-foreground">{{ $t('components.generationParams.lengthSection') }}</div>
                    <div class="grid grid-cols-2 gap-3">
                      <div class="space-y-1.5"><label for="settings-default-min-new-tokens" class="text-[10px] text-muted-foreground">{{ $t('components.generationParams.minNewTokens') }}</label><input id="settings-default-min-new-tokens" name="default_min_new_tokens" type="number" min="1" max="32767" step="1" :value="paramValue('min_new_tokens')" :placeholder="$t('components.generationParams.notSet')" class="param-input" @change="setNumber('min_new_tokens', ($event.target as HTMLInputElement).value, 1, 32767, true)" /></div>
                      <div class="space-y-1.5"><label for="settings-default-max-new-tokens" class="text-[10px] text-muted-foreground">{{ $t('components.generationParams.maxNewTokens') }}</label><input id="settings-default-max-new-tokens" name="default_max_new_tokens" type="number" min="1" max="32767" step="1" :value="paramValue('max_new_tokens')" placeholder="2048" class="param-input" @change="setNumber('max_new_tokens', ($event.target as HTMLInputElement).value, 1, 32767, true)" /></div>
                    </div>
                    <div class="flex items-center justify-between gap-3"><div class="text-xs text-muted-foreground">{{ $t('components.generationParams.nonStreamingMode') }}</div><AppSelect compact :model-value="triStateValue(defaultParams[selectedKind].non_streaming_mode)" :options="triStateOptions" @update:model-value="setTriState" /></div>
                  </section>
                  </div>
                </div>
              </section>
